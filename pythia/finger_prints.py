...
# own modules
from . import graph_hopping as gh
...
def atom_pair_fingerprints(
    smiles, bit_vec=False, return_only_non_zero=False, log_explanation=False
):
    """
    ...
    if log_explanation is True:
        for inx, fp in enumerate(fps):
            log.info(
                "Atom pair finger print number {}:\n\texplanation: {}".format(
                    inx, Pairs.ExplainPairScore(fp)
                )
            )
...
def torsion_fingerprints(
    ...
    if log_explanation is True:
        for inx, fp in enumerate(fps):
            log.info(
                "Topological torsion finger print number {}:\n\texplanation: {}".format(
                    inx, Pairs.ExplainPairScore(fp)
                )
            )
...
def morgan_fingerprints(
    ...
    n_bits=1024,
    bit_vec=True,
    feature_invarients=False,
    return_only_non_zero=True,
    log_explanation=False,
):
    """
    ...
    if bit_vec is True:
        fps = [
            AllChem.GetMorganFingerprintAsBitVect(mol, radius, n_bits, useFeatures=True)
            for mol in mols
        ]

    else:
        fps = [
            AllChem.GetMorganFingerprint(mol, radius, useFeatures=True) for mol in mols
        ]
        if return_only_non_zero is True:
            fps = [fp.GetNonzeroElements() for fp in fps]
    ...
    #     for mol in mols:
    #         explanation = {}
    #         if feature_invarients is True:
    #             if bit_vec is True:
    #                 fps = AllChem.GetMorganFingerprintAsBitVect(mol, radius, n_bits, useFeatures=True, bitInfo=explanation)
    #             else:
    #                 fps = AllChem.GetMorganFingerprint(mol, radius, useFeatures=True, bitInfo=explanation)
    #         else:
    #             if bit_vec is True:
    #                 fps = AllChem.GetMorganFingerprintAsBitVect(mol, radius, n_bits, bitInfo=explanation)
    #             else:
    #                 fps = AllChem.GetMorganFingerprint(mol, radius, bitInfo=explanation)
    #
    #         if return_only_non_zero is True:
    #             fps = fps.GetNonzeroElements()
    #
    #         for key in explanation.keys():
    #             log.info("Bit {}:\n\tdictionary enrty: {}".format(key, explanation[key]))
    #             for tup in explanation[key]:
    #                 sub_graph_env = Chem.FindAtomEnvironmentOfRadiusN(mol, tup[1], tup[0])
    #                 atom_map = {}
    #                 sub_graph = Chem.PathToSubmol(mol, sub_graph_env, atomMap=atom_map)
    #                 log.info("The subgraph for bit {} has {} atoms and is given by mapping: {}".format(key, sub_graph.GetNumAtoms(), atom_map))
    #                 sub_graph_image = Draw.DrawMorganBit(mol, key, explanation, useSVG=True)
    #                 display(sub_graph_image)
    #         all_fps.append((fps, explanation))
    #
    # else:
    #     if feature_invarients is True:
    #         if bit_vec is True:
    #             all_fps = [AllChem.GetMorganFingerprintAsBitVect(mol, radius, n_bits, useFeatures=True) for mol in mols]
    #         else:
    #             all_fps = [AllChem.GetMorganFingerprint(mol, radius, useFeatures=True) for mol in mols]
    #             # if return_only_non_zero is True:
    #             #     all_fps = [fp.GetNonzeroElements() for fp in all_fps]
    #     else:
    #         if bit_vec is True:
    #             all_fps = [AllChem.GetMorganFingerprintAsBitVect(mol, radius, n_bits) for mol in mols]
    #         else:
    #             all_fps = [AllChem.GetMorganFingerprint(mol, radius) for mol in mols]
    #
    #     if return_only_non_zero is True:
    #                 all_fps = [fp.GetNonzeroElements() for fp in all_fps]
    #
    # log.info("Building dataframe .....")
    # df = pd.DataFrame(data=all_fps)
    # log.info("Building RDKit bits .....")
    # all_fps = [DataStructs.cDataStructs.CreateFromBitString("".join([str(ent) for ent in f])) for f in all_fps]
...
def draw_molecule(smiles, smarts=None, filename="tmp.png"):
    ...
    mol = gh.smiles_to_molcule(smiles)
    ...
    if smarts is not None:
        ...
        try:
            match_bonds = [
                mol.GetBondBetweenAtoms(
                    match_atoms[b.GetBeginAtomIdx()], match_atoms[b.GetEndAtomIdx()]
                ).GetIdx()
                for b in substructure.GetBonds()
            ]
        except IndexError:
            no_sub_structure = True
        ...
        if no_sub_structure is False:
            ...
            rdMolDraw2D.PrepareAndDrawMolecule(
                img, mol, highlightAtoms=match_atoms, highlightBonds=match_bonds
            )
        else:
