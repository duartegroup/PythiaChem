...
def molecule_image(mol, smile, fnam=None, label_with_num=True):
    ...
    if label_with_num is True:
        for atom in mol.GetAtoms():
            atom.SetProp(
                "atomLabel", "{}{}".format(atom.GetSymbol(), str(atom.GetIdx()))
            )
...
def mol_grid(
    ...
    if mols is None:
        if smiles is None:
            log.error(
                "Error - Need either list of smiles or list of molecule objects from RDKit"
            )
        else:
    ...
    if filename is None:
        if labels is None:
            grid = Chem.Draw.MolsToGridImage(
                mols, molsPerRow=mol_row, subImgSize=(400, 400), maxMols=max_mols
            )
        else:
            grid = Chem.Draw.MolsToGridImage(
                mols,
...
def twod_mol_align(molecules, template_smarts=None, template_smiles=None):
    ...
    for m in molecules:
        ...
        rms = AllChem.AlignMol(
            m, temp_mol, atomMap=list(zip(mol_atom_inxes, temp_atom_inxes))
        )
