...
def feature_categorization(features_df, feature_types="some_categorical", categorical_indxs=None):
    ...
    if feature_types == "no_categorical":
        ...
        all_feature_indices = [i for i in range(len(features_df.columns))]
        categorical_indices = all_feature_indices if categorical_indxs is None else categorical_indxs
        numeric_indices = [i for i in all_feature_indices if i not in categorical_indices]
        ...
        numerical_transformer = MinMaxScaler()
        categorical_transformer = OneHotEncoder(handle_unknown='ignore', sparse_output=False, categories='auto')
        ...
        categorical_feature_names = categorical_transformer.get_feature_names_out(features_df.columns)

        features_df = pd.DataFrame(transformed_features, columns=categorical_feature_names, index=features_df.index)
        ...
        return features_df, None, None, None, []
...
def scale_test_set(features_df, categorical_indxs, numerical_transformer, categorical_transformer):
    ...
    :param features_df: DataFrame containing the test set features to be scaled and transformed.
    :param categorical_indxs: List of indices for categorical features in the test set.
    :param numerical_transformer: Fitted numerical transformer (e.g., MinMaxScaler) from the training set.
    :param categorical_transformer: Fitted categorical transformer (OneHotEncoder) from the training set.
    :return: DataFrame containing the scaled and transformed features of the test set.
    """
    log = logging.getLogger(__name__)

    # Separate categorical and numerical features
    categorical_features = features_df.iloc[:, categorical_indxs]
    numeric_features = [features_df.columns[i] for i in range(len(features_df.columns)) if i not in categorical_indxs]

    # Transform numerical features
    if numeric_features and numerical_transformer:
        numerical_features_transformed = numerical_transformer.transform(features_df[numeric_features])
        numerical_df_transformed = pd.DataFrame(numerical_features_transformed, columns=numeric_features,
                                                index=features_df.index)
    else:
        numerical_df_transformed = pd.DataFrame(index=features_df.index)

    # Transform categorical features
    if categorical_features.shape[1] > 0 and categorical_transformer:
        categorical_features_transformed = categorical_transformer.transform(categorical_features)

        # Derive the feature names from the OneHotEncoder categories_
        categorical_feature_names = []
        for col, cat_features in zip(categorical_features.columns, categorical_transformer.categories_):
            categorical_feature_names.extend([f"{col}_{i}" for i in range(len(cat_features))])

        # Ensure the order of columns matches the training feature names
        transformed_categorical_df = pd.DataFrame(categorical_features_transformed, columns=categorical_feature_names,
                                                  index=features_df.index)
    else:
        categorical_feature_names = []
        transformed_categorical_df = pd.DataFrame(index=features_df.index, columns=categorical_feature_names)

    # Combine the transformed numerical and categorical features
    scaled_features_df = pd.concat([numerical_df_transformed, transformed_categorical_df], axis=1)

    # Log final DataFrame columns
    log.info(scaled_features_df.columns)
