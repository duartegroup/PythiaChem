...
def max_common_sub_graph(mols):
    ...
    if not mcs.canceled:
        ...
        log.info(f"suggest visiting https://smarts.plus/smartsview to get the meaning of the SMARTS string")
        log.info("\nQuery molecule:")
...
def compare_hashes(known_set, screening_set, exact=True, within=False, verbose=False, substrutcure_search=False):
    """
...
def restricted_anonymous_hash(smiles, restricted_set="[C]", verbose=False):
    ...
    anon_moles = [smiles_to_molcule(smi, threed=False, addH=False, remove_stereo=True) for smi in smiles]
    anon_hashes = [rdMolHash.MolHash(Chem.rdmolops.RemoveHs(m), rdMolHash.HashFunction.AnonymousGraph) for m in anon_moles]
    rag_hashes = [anon_hash.replace("*", restricted_set) for anon_hash in anon_hashes]
    
    if verbose is True:
        log.info("\n".join(["{} {} {}".format(smiles[ith], ent, elt) for ith, (ent, elt) in enumerate(zip(anon_hashes, rag_hashes))]))
    
    return rag_hashes

def show_specific(index, ld_hashes, ld_smiles, screen_hashes, screen_smiles, matches, dataset=None, dataset_label=None, dataset_structure=None, smarts=False, ld_indexes_giving_hashes=None):
    """
    ...
    if ld_indexes_giving_hashes is not None:
        ...
        if smarts is False:
            ...
            log.info("----- Known scafold index {} -----\nGenerated hash ({}) from molecule:".format(index, ld_hashes[index]))
            ...
            if dataset is not None:
                for j in matches[index]:
                    log.info("-- {}: {}  {} --".format(j, dataset[dataset_label].values[j], dataset[dataset_structure].values[j]))
                    mmtmp = smiles_to_molcule(dataset[dataset_structure].values[j], threed=False)
...
def analyze_regio_hashes(hashes, outname=None, draw=False, figsize=(10,10)):
    ...
    log.info("Regioisomer hash counts over the dataset: {}\n".format(aggregate_group_counts))
    
    if draw is True:
        for ind, s in enumerate(regio_hashes):
            log.info("\nRegio smarts: {} Number of occurances in the dataset: {}".format(s, aggregate_group_counts.values[ind]))
...
def search_for_regio_hashes(search_for_hashes, searchable_hashes, outname=None, draw=False, labels=None):
    """
    Analyze a set of regio hashes to look fro most common fragments
    :param hashes: list - rdkit regio hash list
    :param outname: str - name to save plot to 
...
def show_all(literature_hashes, matches, dataset, literature_mols=None, dataset_label_key="zinc_id", dataset_structure_key="smiles"):
    """
    ...
    for i, known in enumerate(literature_hashes):
        ...
        log.info("----- Known scafold index {} -----\nGenerated hash ({}) from molecule:".format(i, known))
        ...
        for j in matches[i]:
            log.info("-- {}: {}  {} --".format(j, dataset[dataset_label_key].values[j], dataset[dataset_structure_key].values[j]))
            mtmp = smiles_to_molcule(dataset[dataset_structure_key].values[j], threed=False)
            log.info("{}\n--\n".format(display(mtmp)))
