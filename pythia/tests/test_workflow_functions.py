...
from workflow_functions import (df_corr, find_correlating_features, permutation_test, general_two_class, constant_value_columns,
                                ...
                                which_are_misclassified, get_feature_names_from_column_transformers, feature_categorization, ensemble)
...
class TestFeatureCategorization(unittest.TestCase):
    ...
    def test_no_categorical(self):
        result_df, categorical_indxs = feature_categorization(self.df, feature_types="no_categorical")
        numeric_columns = self.df.select_dtypes(include=[np.number]).columns.tolist()
        self.assertEqual(result_df.shape, (4, 4))  # Ensure the shape remains the same
        self.assertTrue((result_df[numeric_columns] >= 0).all().all() and (result_df[numeric_columns] <= 1).all().all())
        self.assertIsNone(categorical_indxs)

    def test_some_categorical_provided_indices(self):
        result_df, categorical_indxs = feature_categorization(self.df, feature_types="some_categorical", categorical_indxs=[3])
        expected_columns = ['A', 'B', 'C', 'D_cat1', 'D_cat2', 'D_cat3']
        self.assertEqual(result_df.shape[1], len(expected_columns))  # Check the number of columns
        self.assertListEqual(result_df.columns.tolist(), expected_columns)
        self.assertListEqual(categorical_indxs, [3])


    def test_some_categorical_auto_detection(self):
        result_df, categorical_indxs = feature_categorization(self.df, feature_types="some_categorical")
        self.assertIn(3, categorical_indxs)
        self.assertGreater(result_df.shape[1], self.df.shape[1])


    def test_all_categorical(self):
        result_df, categorical_indxs = feature_categorization(self.df, feature_types="categorical")
        expected_columns = ['A_1', 'A_2', 'A_3', 'A_4', 'B_2', 'B_3', 'B_4', 'B_5', 'C_3', 'C_4', 'C_5', 'C_6', 'D_cat1', 'D_cat2', 'D_cat3']
        self.assertListEqual(result_df.columns.tolist(), expected_columns)
        self.assertIsNotNone(categorical_indxs)


    def test_no_scaling(self):
        result_df, categorical_indxs = feature_categorization(self.df, feature_types="none")
