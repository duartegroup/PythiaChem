...
def accuracy(cm):
    ...
    if isinstance(cm, dict):
        # If the input is a dictionary, use the dictionary values directly.
        ac = (cm["tp"] + cm["tn"]) / (cm["tp"] + cm["tn"] + cm["fp"] + cm["fn"])
    elif isinstance(cm, np.ndarray):
        if cm.ndim == 1:
            raise ValueError("Confusion matrix should have at least 2 dimensions")
        elif cm.shape[0] == 2 and cm.shape[1] == 2:
            # Binary classification
            ac = (cm[0, 0] + cm[1, 1]) / cm.sum()
        else:
            # Multi-class classification
            cm_acc = np.diag(cm) / np.sum(cm, axis=1)
            ac = np.mean(cm_acc)
    else:
        raise TypeError("Unsupported type for confusion matrix. Expected dict or ndarray.")
