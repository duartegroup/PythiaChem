...
def plot_roc_curve(
    ...
    y_test,
    title="ROC Curve",
    x_lab="False Positive Rate (FPR)",
    y_lab="True Positive Rate (TPR)",
    axes=None,
    figsize=None,
    plot_class="all",
    col_map="viridis",
    ...
    for cls in plot_class:
        ...
        (
            ...
            data[cls]["thresholds"],
            data[cls]["auc"],
        ) = roc_curve_data(y_test, probas[:, cls], pos_label=cls)
...
def precision_recall_data(y_test, probs, pos_label=1):
    ...
    log.debug(
        "precision recall analysis class: {}\n\tPrecision:\n\t{}\n\tRecall:\n\t{}\n\tAverage precision: {}".format(
            pos_label, prec, rec, average_precision
        )
    )
...
def plot_pr_curve(
    ...
    for cls in plot_class:
        ...
        (
            ...
            data[cls]["thresholds"],
            data[cls]["ap"],
        ) = precision_recall_data(y_test, probas[:, cls], pos_label=cls)
        col = plt.cm.get_cmap(col_map)(float(cls) / len(plot_class))
        axes.plot(
            data[cls]["recall"],
            data[cls]["precision"],
...
def plot_metrics(
    ...
    if all_classes is True:
        ...
        cmx = cmet.get_multi_label_confusion_matrix(
            df,
        ...
        if roc_curve is True:
            ...
            axs[len(classes)] = plot_roc_curve(
                probabilities,
                df[known_column_name].values,
                axes=axs[len(classes)],
                plot_class="all",
                col_map=col_map,
                fontsize=fontsize,
            )
