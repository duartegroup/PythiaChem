...
from sklearn.preprocessing import (
    StandardScaler,
)  # This is sklearns auto-scaling function
...
def find_correlating_features(
    features,
    targets,
    thresh=0.4,
    plot=True,
    process_non_numeric=True,
    ...
    for ent in features.columns:
        ...
        feat = series.values
        tmp_df = pd.DataFrame(
            data=np.array([feat, targets]).T, columns=[ent, "property"]
        )
        ...
        try:
            ...
            if abs(correlations.loc[ent, "property"]) > thresh:
                ...
                if significance is True:
                    ...
                    log.info(
                        "{}: {:.4f} P: {:.4f} Significant at {:.4f} level? {}".format(
                            ent,
                            correlations.loc[ent, "property"],
                            p_value,
                            sig_level,
                            significant,
                        )
                    ...
                    log.info(
                        "{}: {:.4f}".format(ent, correlations.loc[ent, "property"])
                    )
                ...
                log.debug(
                    "Feature {} does not correlate to the threshold {}".format(
                        ent, thresh
                    )
                )
...
def permutation_test(
    ...
    try:
        ...
        log.info(
            "{}: {:.4f} P: {:.4f} Significant at {:.4f} level? {}".format(
                ent, correlations.loc[ent, "property"], p_value, sig_level, significant
            )
        )
        ...
        # If non-numeric error messages occur from mordred the correlation matrix is empy and has a key error
        log.warning(
            "WARNING - some molecules do not have this feature thus the correlation is for a sub-set."
        )
        tmp_df = tmp_df[pd.to_numeric(tmp_df[ent], errors="coerce").notnull()]
        tmp_df[ent] = pd.to_numeric(tmp_df[ent])
        tmp_df["property"] = pd.to_numeric(tmp_df["property"])
...
def amine_and_aromatic_nsp2_checker(
    smiles,
    ammonia="[NH3]",
    primary="[#7X3;H2][#6X4]",
    secondary="[#7X3;H1]([#6X4])[#6X4]",
    ...
    n_amine_types = (
        has_primary_amine + has_secondary_amine + has_tertiary_amine + has_aromatic_Nsp2
    )
...
def vector_amine_types(
    smiles,
    ammonia="[NH3]",
    primary="[NX3;H2][CX4]",
    secondary="[NX3;H1]([CX4])[CX4]",
...
def amine_aromatic_none_aromatic(
    smiles,
    ...
    log.debug(
        "Amine is aromatic (0 is no 1 is yes) ? {}".format(aromatic_none_aromatic)
    )
        ...
        if units == "molco2_moln":
            ...
            log.info(
                "{} Amine molar capacity (mol co2 / mol amine) threshold {:.2f} capacity {:.2f} class {}".format(
                    indx, ret * number_of_N_atoms[indx], capacity[indx], classes[-1]
                )
            )
            ...
            log.info(
                "{} Mass capacity (co2 (g) / amine (g)) threshold {:.2f} capacity {:.2f} class {}\n----\n".format(
                    indx,
                    (ret * number_of_N_atoms[indx]) / molar_ratios[indx],
                    capacity[indx],
                    classes[-1],
                )
            )
...
def classify(n_primary, n_secodnary, n_tertiary, n_aromatic_sp2):
    ...
    if n_primary + n_secodnary > 0 and n_tertiary > 0:
        log.debug(
            "{} {} {} {}".format(n_primary, n_secodnary, n_tertiary, n_aromatic_sp2)
        )
...
def test_imbalenced_classifiers_with_optimization(
    df,
    classes,
    classifiers,
    clf_options,
    ...
    Xtrain, Xtest, ytrain, ytest = train_test_split(
        data, classes, test_size=test_set_size, random_state=random_seed, shuffle=True
    )
    log.info(
        "Xtrain: {}\nXtest: {}\nytrain: {}\nytest: {}".format(
    ...
    log.info(
        "Test set is made up of:\n{}".format(
            "\n".join(
                [
                    "name {} smiles {} class {}".format(names[j], smiles[j], c)
                    for j, c in zip(Xtest.index, ytest["classes"].values)
                ]
            )
        )
    )
    ...
    log.info(
        "Starting classification: NOTE on confusion matrix - In binary classification, true negatives is element 0,0, false negatives is "
    ...
    for name, classf in zip(clf_names, classifiers):
        ...
        disp = ConfusionMatrixDisplay(
            confusion_matrix=c_matix, display_labels=class_labels
        )
        disp.plot()
        ...
        log.info(
            "{}\n-----\n".format(
                "\n".join(
                    [
                        "{}   |   {}".format(p, k)
                        for p, k in zip(predicted_clf, ytest["classes"].values)
                    ]
                )
            )
        )
...
def kfold_test_imbalenced_classifiers_with_optimization(
    df,
    classes,
    classifiers,
    clf_options,
    ...
    for name, classf in zip(clf_names, classifiers):
        ...
        if not os.path.isdir(name):
            ...
            log.warning(
                "Directory already exists and overwrite is False will stop before overwriting.".format(
                    name
                )
            )
            return None
        ...
        for train_indx, test_indx in kf.split(df):
            ...
            # Grid search model optimizer
            opt_param = grid_search_classifier_parameters(
                classf,
                Xtrain,
                ytrain,
                clf_options,
            ...
            ax.plot(
                ...
                color="white",
                lw=1.5,
                label="pre_class0 = {:.2f}\n".format(output_dict[0]["pre"])
                + "pre_class1 = {:.2f}".format(output_dict[1]["pre"]),
            )
            ax.plot(
                fpr,
                tpr,
                ...
                color="white",
                lw=1.5,
                label="f1_class0 = {:.2f}\n".format(output_dict[0]["f1"])
                + "f1_class1 = {:.2f}".format(output_dict[1]["f1"]),
            )
            ax.plot(
                fpr,
                tpr,
                ...
                color="white",
                lw=1.5,
                label="rec_class0 = {:.2f}\n".format(output_dict[0]["rec"])
                + "rec_class1 = {:.2f}".format(output_dict[1]["rec"]),
            )
                ...
                "\t{}\n-----\n".format(
                    "\n\t".join(
                        [
                            "{}   |   {}   |   {}".format(i, p, k)
                            for i, p, k in zip(
                                test_indx, predicted_clf, ytest["classes"].values
                            )
                        ]
                    )
                )
        ...
        if any(x not in tmp for x in [y for y in range(len(classes.index))]):
            log.info(
                "WARNING there appears to be left over indexes which have not been used for testing: {}".format()
            )
        else:
            log.info(
        ...
        log.info(
            "{} {} {} {}".format(
                average_scores, std_scores, average_roc_auc, std_roc_auc
            )
        )
        ...
        score_str3 = "Average ROC AUCs: {:.2f} +/- {:.2f}".format(
            average_roc_auc, std_roc_auc
        )
...
def build_data_from_directory(data_directory, max_folds=10):
    ...
    for i in range(0, max_folds):
        ...
        data_fold = pd.read_csv(
            os.path.join(data_directory, "{}.csv".format(i)), header=0
        )
        # log.info(data_fold)
        if i == 0:
            data = data_fold.copy()
...
def metrics_for_all_classes(
    ...
    for directory in directories:
        ...
        if "names" or "smiles" in data.keys():
            ...
            log.info(
                "Ideally give at least one of names or smiles to find out which molecules were misclassified"
            )
        ...
        multi_metrics = cmetrics.calculate_multi_label_confusion_based_metrics(
            df=data,
            probabilities=probs,
            positive_label=1,
            imbalanced=True,
            plt_filename=os.path.join(directory, "metrics.png"),
            verbose=False,
        )
        ...
        metrics0.append(
            (round(multi_metrics[0]["matthews_correlation_coefficient"], 2))
        )
        ...
        metrics1.append(
            (round(multi_metrics[1]["matthews_correlation_coefficient"], 2))
        )
        ...
        metrics = cmetrics.calculate_confusion_based_metrics(
            df=data, probabilities=probs, positive_label=1
        )
        ...
        multi_metrics = cmetrics.calculate_multi_label_confusion_based_metrics(
            ...
            positive_label=1,
            imbalanced=True,
            plt_filename=os.path.join(directory, "metrics_real_only.png"),
            verbose=False,
        )
        ...
        log.info(
            "Over all REAL data points NOT including smote points:\n{}".format(metrics)
        )


def which_are_misclassified(
    data,
    known_column_label="known",
    predicted_column_label="prediction",
    return_indx=False,
):
    """
    ...
    if return_indx is False:
        ...
        if "names" in df.keys() and "smiles" in df.keys():
            log.info(
                "The molecules which are misclassified are:\n{}".format(
                    "\n".join(
                        [
                            "{} {}".format(name, smile)
                            for name, smile in zip(
                                df["names"].values, df["smiles"].values
                            )
                        ]
                    )
                )
    ...
    for ith, ent in enumerate(ctransformer.transformers_):
        ...
        log.info(
            "Transformer index: {}\nTranformer name: {}\nTransformer: {}\nOriginal feature names: {}\n".format(
                ith, trans_name, trans, original_feature_name
            )
        )
        if hasattr(trans, "get_feature_names"):
            ...
            missing_features = [
                original_feature_name[ith] + "_missing_flag"
                for ith in missing_indicator_indices
            ]
...
def test_classifiers(
    ...
    Xtrain, Xtest, ytrain, ytest = train_test_split(
        data, classes, test_size=0.1, random_state=random_seed, shuffle=True
    )
    log.debug(
        "Xtrain: {}\nXtest: {}\nytrain: {}\nytest: {}".format(
            Xtrain, Xtest, ytrain, ytest
    ...
    log.info(
        "Test set is made up of:\n{}".format(
            "\n".join(
                [
                    "name {} smiles {} class {}".format(names[j], smiles[j], c)
                    for j, c in zip(Xtest.index, ytest)
                ]
            )
        )
    )
    ...
    for clf in classifiers:
        ...
        log.info(
            "{}".format(
                "\n".join(
                    ["{}   |   {}".format(p, k) for p, k in zip(predicted_clf, ytest)]
                )
            )
        )


def test_classifiers_with_optimization(
    df,
    classes,
    classifiers,
    clf_options,
    ...
    for name, classf in zip(clf_names, classifiers):
        ...
        plt.figure(figsize=(10, 10))
        plt.plot(
            fp, tp, color="red", lw=1.5, label="ROC curve (auc={:.2f})".format(roc_auc)
        )
        plt.plot([0, 1], [0, 1], "k:")
        plt.xlim([0.0, 1.05])
        plt.ylim([0.0, 1.05])
